		...
		events: async function (fetchInfo, successCallback, failureCallback) {
			try {
				...
				if (
					...
					console.error(
						'Events fetched are not in the expected format or missing required properties'
					);
...
function addevent(events) {
	for (let i = 0; i < events.length; i++)
		calendar.addEvent({
			start: events[i].start.date || events[i].start.dateTime,
			end: events[i].end.date || events[i].end.dateTime,
